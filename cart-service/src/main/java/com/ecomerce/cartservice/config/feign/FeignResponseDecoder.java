...
/**
 * Custom Feign Decoder để unwrap ApiResponse từ các services
 * Services trả về ApiResponse<T>, nhưng Feign client cần T trực tiếp
 */
@Slf4j
public class FeignResponseDecoder implements Decoder {
    ...
    public FeignResponseDecoder(ObjectFactory<HttpMessageConverters> messageConverters) {
        ...
        // Add JavaTimeModule để hỗ trợ deserialize java.time.Instant
    ...
    public Object decode(Response response, Type type) throws IOException {
        ...
        // Xử lý OrderResponse từ order-service
        if (typeName.contains("OrderResponse")) {
            return decodeApiResponse(response, type, OrderResponse.class, "order-service");
        }
    ...
    private <T> T decodeApiResponse(Response response, Type type, Class<T> targetClass, String serviceName) throws IOException {
        try {
            ...
            // Convert data field to target class
    ...
    /**
     * Wrapper class để deserialize ApiResponse từ product-service
     */
